import { Vector3, Euler, DataTexture, RGBAFormat, ClampToEdgeWrapping, MirroredRepeatWrapping, MathUtils, DoubleSide } from 'three';
...
class FBXExporter {
    ...
    async export(object, options = {}) {
        ...
        const format = binary ? 'binary' : 'ascii';
    ...
    createModel(object, id) {
        ...
        const model = {
            ...
            Culling: 'CullingOff',
            Properties70: {
                P: this.createModelPropertyList(object.position.toArray(), this.getEulerRotation(object), object.scale.toArray(), this.getRotationOrder(object), object.visible, hasNodeAttribute ? 0 : -1)
            }
        };
        this.objects.Model[id] = model;
    }
    createModelPropertyList(position, rotation, scale, rotationOrder, visible, defaultAttributeIndex = -1) {
        return [
    ...
    createTypedScalarProperty(name, type, value, encodingType, label = '', flag = '') {
        return {
    ...
    createLookAtTargetForObject(object, modelId) {
        ...
        this.objects.Model[targetId] = {
            ...
            Culling: 'CullingOff',
            Properties70: {
                P: this.createModelPropertyList(targetPosition.toArray(), [0, 0, 0], [1, 1, 1], 5, false, -1)
            }
        };
    ...
    getLightType(light) {
        ...
        if (light.isPointLight)
            return 0;
        this.warnOnce(`unsupported-light-type:${light.type}`, `FBX export does not have a direct mapping for ${light.type}; exporting "${light.name || light.type}" as a PointLight.`);
        return 0;
    }
    getModelType(object) {
        if (object.isMesh)
    ...
    createGeometry(geometry, id, materialCount = 1) {
        const geo = {
            id: id,
        ...
        if (polygonCount > 0 && (materialCount > 1 || geometry.groups.length > 0)) {
            const materialIndices = new Array(polygonCount).fill(0);
            for (const group of geometry.groups) {
                ...
                for (let i = 0; i < groupPolygonCount; i++) {
                    ...
                    if (polygonIndex >= materialIndices.length)
                        break;
    ...
    createMaterial(material, id) {
        ...
        // Emissive color
        if (isStandardMaterial && material.emissive) {
        ...
        if (material.opacity !== undefined && (material.transparent || material.opacity < 1)) {
            mat.Properties70.P.push({
                name: 'Opacity',
                type: 'double',
                label: 'Number',
    ...
    createTextures(material) {
        ...
        if (combinedTextures.br) {
            ...
            if (material.map) {
                const textureId = this.getNextId();
                const videoId = this.getNextId();
                this.createTexture(material.map, textureId, videoId, 'DiffuseColor', materialName, 'Diffuse');
                textureInfos.push({ id: textureId, property: 'DiffuseColor' });
            }
            ...
            if (material.roughnessMap) {
                const textureId = this.getNextId();
                const videoId = this.getNextId();
                this.createTexture(material.roughnessMap, textureId, videoId, 'SpecularFactor', materialName, 'Roughness');
        ...
        if (combinedTextures.meo) {
            ...
            if (material.metalnessMap) {
                const textureId = this.getNextId();
                const videoId = this.getNextId();
                this.createTexture(material.metalnessMap, textureId, videoId, 'SpecularColor', materialName, 'Metallic');
                textureInfos.push({ id: textureId, property: 'SpecularColor' });
            }
            if (material.emissiveMap) {
                const textureId = this.getNextId();
                const videoId = this.getNextId();
                this.createTexture(material.emissiveMap, textureId, videoId, 'EmissiveColor', materialName, 'Emissive');
                textureInfos.push({ id: textureId, property: 'EmissiveColor' });
            }
            if (material.aoMap) {
                const textureId = this.getNextId();
                const videoId = this.getNextId();
                this.createTexture(material.aoMap, textureId, videoId, 'AmbientColor', materialName, 'AO');
        ...
        if (material.normalMap) {
            const textureId = this.getNextId();
            const videoId = this.getNextId();
            this.createTexture(material.normalMap, textureId, videoId, 'NormalMap', materialName, 'Normal');
            textureInfos.push({ id: textureId, property: 'NormalMap' });
        }
        if (material.alphaMap) {
            const textureId = this.getNextId();
            const videoId = this.getNextId();
            this.createTexture(material.alphaMap, textureId, videoId, 'TransparencyFactor', materialName, 'Alpha');
            textureInfos.push({ id: textureId, property: 'TransparencyFactor' });
        }
        return textureInfos;
    }
    createTexture(texture, textureId, videoId, textureType, materialName, slotName) {
    ...
    getTextureUvSet(texture) {
        const channel = Number.isInteger(texture.channel) ? texture.channel : 0;
        return channel <= 0 ? 'UVMap' : `uv${channel}`;
    }
    getTextureWrapMode(wrapMode, axis, textureName) {
        if (wrapMode === ClampToEdgeWrapping) {
            return 1;
        }
        if (wrapMode === MirroredRepeatWrapping) {
            this.warnOnce('mirrored-repeat-wrapping', `FBX export does not support MirroredRepeatWrapping; exporting texture "${textureName}" axis ${axis} as RepeatWrapping.`);
        }
        return 0;
    }
    ...
    async createFiles(fbxFileName, fbxData, textures) {
        const files = {};
        if (fbxData instanceof ArrayBuffer) {
            files[fbxFileName] = new Uint8Array(fbxData);
        }
    ...
    prepareTextureForExport(texture) {
    ...
    createCombinedTexture(material, materialName) {
        void material;
        void materialName;
        // Temporarily disable combined texture creation to avoid performance issues
        return {};
    }
    createBRTexture(material, materialName) {
        const baseColorTexture = material.map;
    ...
    createMEOTexture(material, materialName) {
        const metalnessTexture = material.metalnessMap;
        const emissiveTexture = material.emissiveMap;
    ...
    getTextureData(texture, targetWidth, targetHeight) {
    ...
    getFbxTextureTransform(texture) {
    ...
    getImmediateTextureData(texture) {
        if (texture instanceof DataTexture) {
        ...
        if (texture.image && typeof texture.image.src === 'string') {
            const dataUrl = texture.image.src;
    ...
    extractTextureData(texture) {
        if (!this.embedTextures)
            return null;
        return this.getImmediateTextureData(texture);
    }
    getMaxTextureSize(textures) {
    ...
    addConnection(childId, parentId, type, property = null) {
    ...
    ensurePropertyList(prop) {
        ...
        const propertyList = [name, type, label, flag];
    ...
    buildFBXTree(_target) {
        ...
        for (const [type, objects] of Object.entries(this.objects)) {
        ...
        for (const [type, objects] of Object.entries(this.objects)) {
            for (const [id, obj] of Object.entries(objects)) {
                if (!fbxTree.Objects[type])
                    fbxTree.Objects[type] = {};
    ...
    writeObjectFromNode(type, obj) {
        ...
        if (obj.Translation !== undefined) {
            const values = obj.Translation.propertyList
                ? obj.Translation.propertyList.map((entry) => entry.value)
                : obj.Translation;
            str += this.indent(2) + `Translation: ${values.join(',')}\n`;
        }
        if (obj.Scaling !== undefined) {
            const values = obj.Scaling.propertyList
                ? obj.Scaling.propertyList.map((entry) => entry.value)
                : obj.Scaling;
    ...
    writeBinaryNode(writer, name, node, version, depth = 0) {
        ...
        if (node === null) {
            const writeMetaValue = version >= 7500
                ? (value) => writer.setUint64(value)
                : (value) => writer.setUint32(value);
            writeMetaValue(0);
            writeMetaValue(0);
            writeMetaValue(0);
            writer.setUint8(0);
    ...
    collectNodeProperties(node, nodeName) {
        const properties = [];
        ...
        if (node.rawBytes) {
            properties.push({ rawBytes: node.rawBytes });
            return properties;
        }
        ...
        if (node.attrType !== undefined)
            properties.push(node.attrType);
        ...
        if (node.a !== undefined) {
            ...
            if (allowWithOthers) {
                ...
                if (keys.length === 1) {
                    if (node.dataType) {
                        properties.push({ value: node.a, encodingType: node.dataType });
                    }
                    else {
                        properties.push(node.a);
                    }
    ...
    collectNodeSubNodes(node) {
        ...
        for (const [key, value] of Object.entries(node)) {
            ...
            // }
            if (Array.isArray(value)) {
                const treatAsSingleProperty = key === 'LocalTime' ||
                    key === 'ReferenceTime' ||
                    key === 'Translation' ||
                    key === 'Scaling';
                if (key === 'P') {
    ...
    writeBinaryProperty(writer, prop) {
            ...
            if (Array.isArray(value) || (ArrayBuffer.isView(value) && !(value instanceof DataView))) {
                ...
                if (encodingType === 'd') {
                    for (const entry of arrayValues)
                        writer.setFloat64(entry);
                }
                else if (encodingType === 'f') {
                    for (const entry of arrayValues)
                        writer.setFloat32(entry);
                }
                else if (encodingType === 'i') {
                    for (const entry of arrayValues)
                        writer.setInt32(entry);
                }
                else if (encodingType === 'l') {
                    for (const entry of arrayValues)
                        writer.setUint64(entry);
                }
            }
        ...
        if (typeof prop === 'number') {
            ...
            const arrayValues = Array.isArray(prop) ? prop : Array.from(prop);
    ...
    formatNodeAttrName(name, nodeType) {
     ...
     */
    generateMinimalFBX(binary = false) {
