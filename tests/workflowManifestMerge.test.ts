...
describe('mergeUnlinkedManifestEntriesIntoWorkflowAssets', () => {
  ...
  it('将 wf-orig 与 wf-res 合并为一张卡片且 id 与伴侣键一致', () => {
    ...
    // 新命名无步骤 id：slot0=原图，slot1+=synthetic `slot_N`
    const rk = workflowResultCompanionStorageKey(ASSET_ID, STEP, 'jpg', 1);
    const m = man([
    ...
    expect(nextAssets[0]!.resultsCompanionKeys?.slot_1).toBe(rk);
    expect(nextAssets[0]!.resultOrder).toContain('slot_1');
  ...
  it('manifest 仅有 wf-res（无 wf-orig 条目）时仍按 UUID 前缀合并', () => {
    // slot0 无步骤名时归为 original；slot1 归为 synthetic result
    const rk1 = workflowResultCompanionStorageKey(ASSET_ID, 'step_a', 'jpg', 0);
    const rk2 = workflowResultCompanionStorageKey(ASSET_ID, 'step_b', 'jpg', 1);
    const m = man([
    ...
    expect(nextAssets.length).toBe(1);
    expect(nextAssets[0]!.id).toBe(ASSET_ID);
    expect(nextAssets[0]!.originalCompanionKey).toBe(rk1);
    expect(nextAssets[0]!.resultsCompanionKeys?.slot_1).toBe(rk2);
  });
