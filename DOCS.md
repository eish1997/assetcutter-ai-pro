...
| 模块       | 入口/模式   | 主要功能 |
|------------|-------------|----------|
| 贴图工坊   | `AppMode.TEXTURE`  | 图案提取、无缝循环贴图 |
...
侧栏点击「贴图工坊」进入。贴图步骤有：**图案提取（T_PATTERN）**、**无缝循环贴图（T_TILE）**；PBR（T_PBR）在类型和配置中存在，当前 UI 未单独展示，可在 `TextureEngineSection` 中按需扩展。
...
- **分组展示**：按 `groupId` 分组，每组用 **LibraryCard** 展示：
  ...
  - **发送到**（按格式限制）：**图片**（有预览图且非占位）可发送到「继续编辑」（→ 对话生图，预填输入图）、「贴图」（→ 贴图工坊，预填源贴图）、「生成3D」（→ 生成3D 图生，预填参考图）；**3D 模型**（有 modelUrls）可发送到「生成3D 中使用」（→ 生成3D，预填智能拓扑/纹理/组件/UV/转换的模型 URL，默认切到智能拓扑）。
