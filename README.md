...
| **工作流** | 多图筛选 → 拖拽/点选到功能框 → 待处理 → 一键执行 → 版本切换与归档；文本类资产支持居中灯箱查看与 Prompt 微调弹窗；工作区支持多窗格分栏与侧栏扩展（浮动坞） |
...
| **设置** | API 密钥（Gemini 等）、能力商店地址、入站密码，以及对话生图本地中转（A-Driver）开关与设备 ID |
...
| 11. **本地伴侣**（可选） | `npm run local-companion:dev` | 本机 `127.0.0.1:18765` 浏览器控制台：**插件**（计算等占位）与 **仓库**（卷路径）；与主站「设置 → 本地伴侣」探测对齐；勿与 A-Driver `local-bridge` 同端口并行 |
