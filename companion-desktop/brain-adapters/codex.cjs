...
function buildToolContext(input) {
  ...
  const lines = [
    ...
    '- If the user asks to add/create a software connection, treat every target app as unknown first. Prefer ac.capability.create_draft with type=software_connection and natural-language intent; do not ask the user to choose a technical template or pick a known host category.',
    '- Known apps such as Photoshop, Maya, Blender, Spine, Unity, and Unreal are only examples with verified strategy shortcuts. Software connection support is extended through connectionFacts -> StrategyDraft/candidateStrategies -> softwareBridgeRegistry/bridge driver model; do not suggest editing capabilityLifecycle.ts for a new app. Treat ac.companion.host_bridge.* as legacy recovery/debug tools, not the default creation path.',
    '- For an existing software connection that needs ongoing repair, installation, launch, or verification, prefer ac.capability.connection_loop_run with explicit bounded permissions. First read that CapabilityPackage context/connectionState, connectionFacts, StrategyDraft, and candidateStrategies; keep all work scoped to that object session, do not switch back to the legacy 62-host catalog or ac.companion.host_bridge.*, and never mark the connection successful without a real probe signal.',
