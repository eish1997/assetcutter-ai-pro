    ...
    if (uri === 'assetcutter://mcp/quickstart') {
      return {
        ...
        text: [
          ...
          '12. Read `assetcutter://mcp/server-status` before workbench E2E validation. Check `readiness.codexRuntime.readyHint`, `readiness.workbenchEntrance.status`, `readiness.account.loggedIn`, `readiness.account.partition`, and `readiness.lastWorkbenchE2eFreshness`; after the embedded workbench is logged in, run `npm run smoke:agent-mcp:e2e -- --config <codex-mcp-import.json>` to verify create project -> run capability -> list assets -> get asset. If login may happen during validation, use `npm run smoke:agent-mcp:e2e:wait-login`.',
        ...
        text: JSON.stringify(
          {
            ...
            readiness: {
              ...
              e2eCommand: 'npm run smoke:agent-mcp:e2e -- --config <codex-mcp-import.json>',
              waitLoginE2eCommand: 'npm run smoke:agent-mcp:e2e:wait-login -- --config <codex-mcp-import.json>',
              recoveryTools: {
