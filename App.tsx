    ...
    <div className="min-h-[100dvh] bg-[#050505] text-white flex flex-col lg:flex-row relative font-sans overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-[100dvh] overflow-hidden">
        <header className="h-16 lg:h-20 glass border-b border-white/5 flex items-center justify-between px-6 lg:px-10 shrink-0 relative z-50">
          ...
          <h2 className="text-[10px] font-black mono text-blue-400 uppercase tracking-[0.5em] truncate">{mode === AppMode.TEXTURE ? '贴图工坊' : mode === AppMode.DIALOG ? '对话生图' : mode === AppMode.GENERATE_3D ? '生成3D资产' : mode === AppMode.ADMIN ? '提示词效果' : mode === AppMode.ARENA ? '提示词擂台' : mode === AppMode.WORKFLOW ? '工作流' : mode === AppMode.CAPABILITY ? '能力' : '资产仓库'}</h2>
        </header>
