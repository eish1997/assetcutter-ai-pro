...
> **MVP 任务书已完成，不要当开工真源。** 后续对象系统见 [`本地伴侣-Workflow对象系统开发文档.md`](./本地伴侣-Workflow对象系统开发文档.md)。产品归属：工作流是楼里独立租户，见 [`架构宪章-本地壳大楼租户.md`](./架构宪章-本地壳大楼租户.md)。

## 0. 本文定位

这是当时把 Workflow 页推到 Maya FBX MVP 的循环任务书，账本已写到「下一任务已完成」。新会话不要从这里起任务。
