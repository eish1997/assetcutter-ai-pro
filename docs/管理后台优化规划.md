...
| **AI 代理** | Gemini/Vertex，公平排队 | `server/gemini-proxy-api.js` |
...
- [ ] 系统：**经 auth-api 聚合** auth + gemini-proxy health（避免浏览器跨域直打 proxy，见 §13.2）
...
### 13.1 Gemini 限流配置：auth-api 与 gemini-proxy 分机部署

**现状**：管理 UI 经 **auth-api** 读写 `server/data/gemini-fairness-config.json`；**gemini-proxy** 在**本机进程**读同名路径并约 3s 重读。

| 环境 | 行为 | 风险 |
|---|---|---|
| 本地 `dev:auth-backend` + `dev:gemini-proxy` 同仓库 | 共享 `server/data/`，**生效** | 低 |
| Render **auth-api** 与 **gemini-proxy** 分机 | 各写各的 ephemeral 磁盘 | **管理页改限流对 proxy 不生效** |
...
静态 admin 页跨域请求 **gemini-proxy `/healthz`** 可能遇 CORS。应 **`GET /api/admin/dashboard`** 由 auth-api **服务端 fetch** proxy health 再返回。
