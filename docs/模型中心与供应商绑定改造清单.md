...
- `docs/多模型可运营改造计划.md` — 阶段 1～3 已落地；本文是下一阶段的**工程清单**
- `services/modelRegistry/README.md` — 现有注册表模块说明
...
4. **生图已半对齐**（`getAIForImageModel(registryId)`）；**文本/理解仍走** `getAiProvider()` 单选，需统一到同一套 `pickBinding` 机制。
...
| 项 | 现状 | 目标 |
|----|------|------|
| 生图模型列表 | ✅ `services/modelRegistry/imageModels.ts` | 保持，扩展元数据 |
| 生图调用 | ✅ `getAIForImageModel(registryId)` 按 `providerRoute` 要 Key | 按 **binding 优先级** 选 channel |
| 文本/理解 | ❌ `getAiProvider()` + `getAI()` 单通道 | 文本也走 `registryId` + binding |
| 设置 UI | 多选「供应商」平铺（`AiProviderCredentialsPanel`） | 按 **模型族 / channel 接线** 管理 |
| 上游 id | `resolve.ts` 按 `AiProvider` 枚举映射 | 按 **binding.channel** 映射 |
| 中转站 | ToAPIs / VectorEngine 与 Vertex 平级 | 声明 `supportedFamilies`，只给已接路径挂 binding |
| 云同步 | `aiProvider` + `enabledAiProviders` + 各 Key | 增加 `enabledChannels` / `providerBindings` |
...
建议新增 `services/modelRegistry/types.ts`：
...
| # | 任务 | 产出 |
|---|------|------|
| A.1 | ADR：模型 SKU vs 供应商 Binding | `docs/adr/模型中心与供应商绑定.md`（可选，本文 §2 可兼 ADR 摘要） |
| A.2 | 定义 `ModelFamily` / `ChannelId` / `ProviderBinding` | `services/modelRegistry/types.ts` |
| A.3 | 冻结首期 `ChannelId` 枚举 | 见 §4 |
...
- [ ] 新同学 30 分钟内能回答：「加一家新中转 = 加 binding，不是加菜单模型」
- [ ] `ChannelId` 与现有 `AiProvider` 映射关系有表格说明
...
- [ ] 只改 `providerBindings.ts` + `imageModels.ts` 即可在 merge 层看到新模型行
- [ ] `npm run typecheck` 通过
- [ ] 单测：`tests/providerBindings.test.ts`（binding 覆盖所有 `registryId`）
...
| C.2 | `isChannelReady(channel)` | 凭证 + env 检查 | `settingsStore.ts` 或 `channelCredentials.ts` |
| C.3 | `resolveUpstreamModelId(binding, role)` | 从 `resolve.ts` 的 provider 分支迁到 channel 分支 | `resolve.ts` |
| C.4 | 生图：`getAIForImageModel` → `getClientForRegistry(registryId)` | 内部 pickBinding + adapter | `geminiService.ts` |
| C.5 | 保留 `resolveUpstreamImageModelIdForRegistry` 薄封装 | 兼容旧调用 | `resolve.ts` |
| C.6 | 日志：`[model-registry] picked binding=… channel=…` | 可观测 | `log.ts` |

**验收（#accept-phase-binding-c）**：

- [ ] `tests/pickBinding.test.ts`：仅 Vertex ready → 选 vertex；Vertex 不可用 → fallback ToAPIs
- [ ] 手测：关 AI Studio、开 Vertex，同一「Gemini 3 Pro Image」仍能生图
- [ ] 无「先 resolve 再 adapter 又改一次」的双重映射（review `resolve` vs `toapisAdapter`）
...
| D.4 | 持久化 `ac_enabled_channels` 或扩展现有 enabled 列表 | `settingsStore.ts` | |
| D.5 | 云配置 `enabledChannels` / `providerBindings` | `workspaceUserCloudConfig.ts`、`App.tsx` | |
| D.6 | 工作流密钥弹窗与设置页共用 panel | 已部分完成 | `WorkflowApiKeyModal.tsx` |

**验收（#accept-phase-binding-d）**：

- [ ] 用户看不到「试用 / Antigravity」
- [ ] 可同时启用 Vertex + ToAPIs，状态分别显示
- [ ] 换设备后 channel 启用状态一致
- [ ] 无新增原生 `<select>`（`.cursor/rules/dropdown-ui-style.mdc`）
...
| E.3 | 全站 `resolveUpstreamTextModelId` 带 binding | 广搜替换 | `capabilityExecutor` 等 |
...
- [ ] 理解 + 生图可配置不同 channel（若产品允许）或同 family 无感 failover（若产品选定）
- [ ] 回归：文本 + 生图各至少一条主路径烟测
...
| # | 任务 | 产出 |
|---|------|------|
| F.1 | `tests/pickBinding.test.ts`、`tests/channelReady.test.ts` | 单测 |
| F.2 | 扩展 `tests/modelRegistryResolve.test.ts`（按 channel） | 解析 |
| F.3 | 涉及 workflow 时间线时跑 `npm run test:workflow-rings` | 回归 |
| F.4 | `public/model-ops.example.json` 增加 `bindingOverrides` / 紧急下架 channel | 运营 |
| F.5 | PR 门禁：新模型不得硬编码在 `App.tsx`；必须 registry + binding | Review |
...
| **P2** | `docs/adr/模型中心与供应商绑定.md`（可选） | `docs/多模型可运营改造计划.md`、`workflowAiPickIndex.ts` |
| **P2** | `tests/pickBinding.test.ts` | `tests/modelRegistryResolve.test.ts` |
...
1. **同一模型多条 binding 同时启用**：固定优先级 failover，还是允许用户指定「生图优先 Vertex」？
2. **文本与是否必须同 channel**：理解能否走 ToAPIs 而生图走 Vertex？（技术可行，需产品口径。）
