...
**管理后台入口密码（可选）**：通过环境变量 `VITE_ADMIN_PASSWORD` 设置后，访问任意 `/admin` 路径会先进入独立的管理员密码页，输入正确密码（会话内有效，存 `sessionStorage`）后才展示管理后台。未设置则无需二次密码。侧栏提供「退出管理员」可清除会话并返回主站。
...
# 可选：管理后台密码。设置后访问 /admin 需先输入此密码（会话内有效）
VITE_ADMIN_PASSWORD=
```
