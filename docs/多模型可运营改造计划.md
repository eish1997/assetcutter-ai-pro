...
5. **分层宪章**：产品向的「供货商 → 仓库 → 菜单 / 拣货」原则与**非目标**（避免单文件/单表教条）见 `**docs/架构宪章-店仓菜单.md`**，与本计划互补：本计划偏**落地阶段与工程清单**，宪章偏**长期边界与评审口径**。
...
**填表模版**（可直接复制到 PR / issue）：`[docs/spec/phase0-model-inventory-template.md](spec/phase0-model-inventory-template.md)`。
...
| 0.3 | §3.3 能力矩阵表已填全，且与产品确认「每个场景用哪个槽位」                                                                      | 表内无空白行或已标注「不适用」；见 `docs/spec/model-capability-matrix.md` |
...
### 1.4 与《架构宪章》的对照、货架地图与待办

**比喻与五条原则**见 `**docs/架构宪章-店仓菜单.md`**。本节把「多模型 + 可运营」剩余工作与宪章对齐，并给出**可勾选**的评审口径；与 §8 backlog 互补（§8 偏执行优先级，本节偏**边界与完整性**）。

#### 1.4.1 宪章原则 × 现状 × 待办（优先级）
...
| **新模态=新货架契约+菜单**       | `generate_video` / `generate_3d` 已有独立契约与 UI                   | 新模态 PR 必须带：类型字段 + 预设/侧栏 + 矩阵表一行（`docs/spec/model-capability-matrix.md`） | P0  |
...
| **多闸门、禁绕路**            | 网关 + `generate3d/` + 视频桥                                      | PR 评审对照宪章 §2；例外须在 PR 描述写明                                               | P0  |


#### 1.4.2 货物大类 → 仓库闸门 → 运营/配置手段


| 货物大类             | 主要仓库闸门（代码）                                                                                                 | 运营/配置手段（当前）                                                              | 备注                                   |
| ---------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------ |
...
- 已更新 `**docs/spec/model-capability-matrix.md**`（若涉及新场景或新槽位）。
...
- **架构宪章与 PR**：涉及**新供货商、新产出模态、新仓库闸门或新用户可达调用链**的 PR，须对照 `**docs/架构宪章-店仓菜单.md`** §2 与本文 **§1.4.4** 自检清单；并视情况更新 `**services/workflowAiPickIndex.ts`**。
...
| 优先级 | 方向                 | 说明                                                                                                                                                                                                                                                            |
| --- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0  | **宪章对照 + 阶段 0 收口** | 落实 **§1.4** 评审口径；路径索引 `**workflowAiPickIndex.ts`** 与 PR 自检。阶段 0：矩阵/盘点/策略书面收口（见 [验收标准 — 阶段 0](#accept-phase-0)）；Runbook 见 `**docs/model-ops-runbook.md**`。                                                                                                     |
...
| P2  | **观测与限流**          | **调试日志**：`**VITE_DEBUG_UNIFIED_AI=1`** 时 `**[unified-ai]`** 输出耗时 + `**provider`**（`getAiProvider()`）+ `**registryId`**（传入 model）+ 失败时 `**errorHint**`（`rate_limit` / `upstream_busy` / `auth_config` / `other` 启发式）；生图多参考带 `**refImageCount**`。后续可接远端打点 / 限流。 |
...
| 2026-05-06 | 新增 `**docs/model-ops-runbook.md`**；扩充 `**docs/spec/model-capability-matrix.md`**；§8 P0 行改为指向上述文档                                                                                                                                                                 |
...
| 2026-05-06 | 阶段 0 模版 `**docs/spec/phase0-model-inventory-template.md**`；验收阶段 0 链入；`**model-capability-matrix.md**`、`**DOCS.md` §11.2** 引用                                                                                                                                     |
