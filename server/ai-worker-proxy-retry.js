/** Shared retry classification for ai-worker-proxy (429/503/504 etc.). */
...
export function aiWorkerProxyRetryDelayMs(err, attempt) {
...
export function aiWorkerProxyMaxAttempts(err, overloadMaxAttempts) {
  if (isUpstreamRateLimitError(err)) {
    const n = Number(process.env.AI_WORKER_PROXY_RATE_LIMIT_RETRIES);
