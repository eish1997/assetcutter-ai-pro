      ...
      {hasDropContent ? (
        <div className="flex flex-col gap-1.5">
          ...
          >
            ...
            附件
            {(mainDropSlots.length + referenceDropSlots.length + promptCards.length) > 0 ? (
              <span className="tabular-nums text-gray-600">
                ({mainDropSlots.length + referenceDropSlots.length + promptCards.length})
              </span>
            ) : null}
          </button>
              ...
              {promptCards.length > 0 && onRemovePromptCard ? (
                <div className="flex flex-wrap items-center gap-2 px-2 pt-2">
                  {promptCards.map((c) => (
                    <div
                      key={c.key}
                      className="group inline-flex max-w-full min-w-0 shrink-0 items-center gap-1.5 rounded-lg bg-white/[0.04] px-2 py-1 ring-1 ring-white/[0.08]"
                      title={c.instruction.trim() ? c.instruction : c.label}
                    >
                      <span className="min-w-0 truncate text-[12px] text-gray-100">{c.label}</span>
                      <button
                        type="button"
